function [Ahorro_Total,E_F]=MR_HNMED6 (E,num_simplex,e)
...
%sFO=zeros(ejecuciones,2); %FO y Número de corrida
    ...
    for hora=1:HORAS    %Cada hora representa una problema de optimización diferente      
        ...
        X=zeros(NP,size(cotas,1)+5);
        X=generate_border_simplex(num_simplex,cotas);
        ...
        while Evaluaciones<E
          ...
          [F,CR]=generate_F_and_CR();% Cálculo del factor de cruza. Este factor es por ejecución o corrida
            ...
            while w<NP
                ...
                xnew=r1(1:var)+F*(best(1:var)-r1(1:var))+(1-F)*(r2(1:var)-r3(1:var));
                xnew(1:var)=limit(xnew(1:var),cotas);
                ...
                [xr1,xr2,xr3]=generate_r1_r2_r3(w,X);
                rk=randi(num_simplex);
                xlk=XL(rk,:);
                padre=Sk(m,:);
                ruido=padre(1:var)+F*(xlk(1:var)-xr1(1:var))+(1-F)*(xr2(1:var)-xr3(1:var));
            ...
            g=g+1;
            m=m+1;
            if primer_factible==0 && best(var+5)==0
                primer_factible=g;
            end
        end
%          Descomentar para observar los resultados por hora        
%          display(['Mejor Solución: ',mat2str(best)]);
%          display(['Generaciones: ',num2str(g)]);
%          display(['Evaluaciones: ', num2str(Evaluaciones)]);
%          display(['Primer_Factible_En_Generacion: ',num2str(primer_factible)]);
...
function [X]=generate_border_simplex(NS,rango)
n=get_problem_n();
rng(0,'v5uniform');
a=0.05;
b=3*n/100;
...
while k<=NS
    ...
    for i=1:n+1
        ...
        for j=1:n
            if i==j
                B=c+(d-c)*rand(1);
                S(i,j)=((rango(j,2)-rango(j,1))/2+((rango(j,2)-rango(j,1))/2)*B);
            else
                if rand()<0.5
                    S(i,j)=rango(j,1)+(((rango(j,2)-rango(j,1))/2)*A) +(NS*(n+1)/100*rand());
                else
                    S(i,j)=rango(j,1)+(((rango(j,2)-rango(j,1))/2)*A) -(NS*(n+1)/100*rand());
                end
            end
