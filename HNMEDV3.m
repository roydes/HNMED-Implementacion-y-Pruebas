function [best,E_F]= HNMEDV3(NE,NS)
...
while E<NE
    ...
    while w<NP
        ...
        %*******If xnew no better than worst then apply V3 operators*******
         xnew1=xl(1:n)-F*(xh(1:n)-best(1:n));
         xnew1=limit(xnew1(1:n),bounds);
         xnew1=evaluate_x_i(xnew1,n);
         E=E+1;
         
         
         xnew2= xl(1:n)+F*(best(1:n)-xh(1:n));
         xnew2=limit(xnew2,bounds);
         xnew2=evaluate_x_i(xnew2,n);
         E=E+1;
         
         
         if compare_by_rules(xnew1,xnew2)
             Sk(n+1,:)=xnew1;
         else
             Sk(n+1,:)=xnew2;
         end
        ...
        if compare_by_rules(Sk(n+1,:),xl)    xl=Sk(n+1,:); end 
                XL(k,:)=xl; 
        if compare_by_rules(Sk(n+1,:),best)   best=Sk(n+1,:);end
