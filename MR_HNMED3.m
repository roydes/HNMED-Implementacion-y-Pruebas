function [Ahorro_Total,E_F]=MR_HNMED3 (E,num_simplex,e)
...
%sFO=zeros(ejecuciones,2); %FO y Número de corrida
    ...
    NP=(var+1)*num_simplex; %Cantidad de individuos de la población.
    MAX_GEN=240;
        ...
        while Evaluaciones<E
          ...
          [F,CR]=generate_F_and_CR();% Cálculo del factor de cruza. Este factor es por ejecución o corrida
            ...
            while w<NP
                ...
                %_______ xnew no better than worst then random expansion')
                ...
                xnew1= xl(1:var)-F*(xh(1:var)-best(1:var));
                %xnew= (1+rand(1)*n)*xc(1:n)+F*(best(1:n)-XL(randi(num_simplex,1),1:n));
                xnew1(1:var)=limit(xnew1(1:var),cotas);
                
                % Evaluar FO y SVR_________________________________________
                %xr=evaluate_x_i(xr,n);
                [xnew1(var+1)]=funcionObjetivo(xnew1(1:var)');
                %Calculando la restricción de igualdad
                %P1_k+P2_k+P3_k+P4_k-PL_k=0
                %utilizaremos un epsilon "cero gordo" de 1E-6
                xnew1(var+2)=sum(xnew1(1:var))-PL;
                %Fin de Calculando la restricción de igualdad.
                
                %Evaluación de las restricciones de desigualdad
                Lizq1=alpha_d*xnew1(2)-alpha_c*xnew1(3)-alpha_c*xnew1(4);
                xnew1(var+3)=Lizq1-b_NNO;
                Lizq2=-alpha_d*xnew1(2)+alpha_c*xnew1(3)+alpha_c*xnew1(4);
                xnew1(var+4)=Lizq2-b_DO;
                %Fin de evaluación de las restricciones de desigualdad
                
                %Calculando la SVR (Suma de violación de restricciones) de cada
                %individuo, Si la casilla tiene un cero, el individuo es factible.
                xnew1(var+5)=0;
                if abs(xnew1(var+2))>0.000001
                    xnew1(var+5)=abs(xnew1(var+2));
                end
                for j=var+3:var+4
                    if xnew1(j)>0
                        xnew1(var+5)=xnew1(var+5)+xnew1(j);
                    end
                end
                Evaluaciones=Evaluaciones+1;
                %_____________________________________________________________________________
                    if compare_by_rules(xnew1,xnew)
                    Sk(var+1,:)=xnew1;
                    else
                    Sk(var+1,:)=xnew;
           ...
           %Si el individuo es factible se almacena en la matriz
            ...
            if(best(var+5)==0)
                ...
                bestGen=best; %variable auxiliar
                bestGen(var+6)=sum(best(1:4)); %carga
                bestGen(var+7)=g;
                bestGen(var+8)=Evaluaciones;
                bestGen(var+9)=NP;
                mejoresGen=vertcat(mejoresGen,bestGen); %concatenación vertical
            end
            if hora==11
