...
x=evaluate(x,n);
while I<NI
    %Sorting points in descend acording the objective function value
    ...
    if compare_by_rules(xnew,xh)
     ...
     x(1,:)=xnew;
      ...
      %disp('no better then ramdon expansion')
      xnew= (1+rand(1)*n)*xc(1:n-1)-rand(1)*xh(1:n-1);
      xnew(1:n-1)=limit(xnew(1:n-1),rango);
      xnew=evaluate_x_i(xnew(1:n-1),n);
      x(1,:)=xnew;
    end
...
function [x]=generate_simplex(n, rango)
for j=1:n-1
   x0(j)=(rango(j,1)+(rango(j,2)-rango(j,1)))/2;
   %x(i,j)=0
end
v = randn(n,n-1);
u = bsxfun(@rdivide,v,sqrt(sum(v.^2,2)));
for i=1:n
   x(i,:)=x0+u(i)*0.5;
   %x(i,j)=0
end
