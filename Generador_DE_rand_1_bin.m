function MR_DE_rand_1_bin
clc
clear 
format long

ejecuciones=31; % ejecuciones para todo el algoritmo
...
for e=1:ejecuciones   
    ...
    NP=50; %Cantidad de individuos de la población del algoritmo heurístico.
    ...
    GlobalFolderName='/Ejecuciones_DERand1Bin_12k';
    ...
    for hora=1:HORAS    %Cada hora representa una problema de optimización diferente      
        ...
        nombreArchivo=[PathFolder ['/DERand1Bin_Mejores_Hora' num2str(hora-1) '.xlsx']];
        encabezado = {'P1'  'P2'  'P3'  'P4'  'FO'  'R1' 'R2' 'R3' 'SVR' 'Carga' 'Generación' 'Evauaciones' 'NP'};
        xlswrite(nombreArchivo,encabezado,1,'A1');
        xlswrite(nombreArchivo,mejoresGen,1,'A2');
        

        %Grafica
        figure(hora);
        q=plot(mejoresGen(:,var+8),mejoresGen(:,var+1));
        %set(p,'linewidth',2);
%         set(gca,'YScale','log');
%         set(gca,'XScale','log');
        ...
        title('Gráfica f(x) vs NFE')
        xlabel('NFE');
        ylabel('f(x)');
        saveas(hora,fullfile(PathFolder, ['Grafica_Convergencia_Hora' num2str(hora-1)]), 'png');
        x_k=best; 
